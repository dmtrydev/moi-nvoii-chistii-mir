...
Эта подсистема:
- подбирает ИНН только из **одобренных** записей в нашей таблице `licenses`
  (`status = 'approved'`), чтобы не ходить в реестр по черновикам и отклонённым;
- держит в БД актуальный «статус лицензии в реестре РПН» по каждому такому ИНН;
...
Добавить строку (запуск **каждый день в 01:00** по серверному времени):

```cron
0 1 * * * /opt/moinoviichistiimir/parser/sync_rpn.sh >> /var/log/rpn-sync.log 2>&1
```

Раньше в примере было `03:00` — актуальное расписание см. выше; при желании
можно оставить любой час, главное — один запуск в сутки на непересекающихся
job’ах.
