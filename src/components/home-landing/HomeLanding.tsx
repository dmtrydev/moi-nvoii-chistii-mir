...
export function HomeLanding(): JSX.Element {
  ...
  /** Карта — после окончания вводной анимации блока «Экологический фильтр» (тот же delay + duration, что у фильтра) */
  ...
  useEffect(() => {
    ...
    const ms = HOME_INTRO_DELAY_FILTER_MS + HOME_INTRO_MOTION_MS;
  ...
  return (
    <section className="relative flex min-h-screen w-full max-w-full min-w-0 flex-col self-stretch">
      ...
      >
          ...
          <div
            className={`relative z-10 mx-auto w-full min-w-0 max-w-[1920px] px-4 ${hasSearched || validationError ? 'pb-12 sm:pb-16' : 'pb-0'}`}
            style={{
              opacity: motionOn ? 1 : 0,
              transform: motionOn
                ? `translateY(${searchLiftPx ? -searchLiftPx : 0}px)`
